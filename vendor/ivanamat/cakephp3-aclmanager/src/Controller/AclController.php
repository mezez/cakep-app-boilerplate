...
class AclController extends AppController {
    ...
    public function revokePerms() {
        ...
        foreach ($models as $model) {
            if ($mCounter == (count($models) - 1)) {
                $f = $this->{$model}->find('all', ['order' => [$model . '.id' => 'ASC']
                        ])->first();
    ...
    private function _parseAros($aros) {
        ...
        foreach ($aros as $aro) {
            ...
            if (isset($aro->title)) {
                $data[$this->model]['title'] = $aro->title;
            }
    ...
    public function manageAro($foreign_key, $model = "Groups") {
        ...
        $Aro = $this->{$model};
        $arosRes = $Aro->find("all")->where(["id" => $foreign_key]);
        // var_dump($arosRes->toArray());
        $aros = $this->_parseAros($arosRes);
        $permKeys = $this->_getKeys();
